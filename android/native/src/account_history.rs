...
pub async fn fetch_locked_vault_devnet_history(
    vault_json: &str,
    ...
    fetch_devnet_history(&account.address().to_string()).await
}

async fn fetch_devnet_history(
    address: &str,
) -> Result<Vec<DevnetSignatureRecord>, HistoryError> {
    ensure_tls_provider();
...
mod tests {
    ...
    fn rejects_empty_vault_json() {
        let result = futures_free_empty_vault_check("   ");

        assert_eq!(result, Err(HistoryError::EmptyVaultJson));
    }
    ...
    fn request_is_devnet_read_only_and_bounded() {
        let address = "BXdqgYBiCMJgCMTm52f8ueVUodQcGTWy24MKfK24ARMw";
        let request = build_get_signatures_for_address_request(address);
    ...
    #[test]
    fn parser_never_returns_more_than_history_limit() {
        let records = (0..15)
            .map(|slot| GetSignaturesForAddressResult {
        ...
        assert_eq!(result.map(|records| records.len()), Ok(HISTORY_LIMIT));
    }

    fn futures_free_empty_vault_check(
        vault_json: &str,
    ) -> Result<Vec<DevnetSignatureRecord>, HistoryError> {
        if vault_json.trim().is_empty() {
            return Err(HistoryError::EmptyVaultJson);
        ...
        Err(HistoryError::VaultParseFailed)
    }
}
